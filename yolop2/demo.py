...
def detect(opt,file_name):
    ...
    source, weights,  save_txt, imgsz = file_name, opt.weights,  opt.save_txt, opt.img_size
    ...
    model=torch.jit.load(weights,map_location=torch.device('cpu')).to(torch.device('cpu'))
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if save_img:
                if dataset.mode == 'image':
                    ...
                    upload_to_s3(save_path, bucket_name)
