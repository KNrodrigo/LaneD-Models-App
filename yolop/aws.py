...
def detect(cfg, opt):
    ...
    for i, (path, img, img_det, vid_cap, shapes) in tqdm(enumerate(dataset), total=len(dataset)):
        ...
        # AWS S3 Integration: Upload output file to S3
        output_bucket = 'nawordth-rodrigo'
        output_key_prefix = 'output/'
        output_s3_key = f"{output_key_prefix}{os.path.basename(save_path)}"
        s3 = boto3.client('s3')
        s3.upload_file(save_path, output_bucket, output_s3_key)
...
if __name__ == '__main__':
    ...
    local_path = os.path.join(opt.output_dir, os.path.basename(s3_key))
